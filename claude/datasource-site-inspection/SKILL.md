...
description: >-
  ...
  VERIFIED / PARTIALLY_VERIFIED / BLOCKED report. Do not use before relevancy
  analysis unless the user explicitly requests a standalone inspection.
---
...
## Tools to Use
1. **Web fetching** — try the download page URL and any direct file URLs first.
2. **Web search** — fallback if the download page is JS-rendered or returns no content.
...
- If it's a large flat file (TSV/CSV/JSON), fetch just the header row or first few lines using a range/byte request if possible, or search for any published schema documentation
